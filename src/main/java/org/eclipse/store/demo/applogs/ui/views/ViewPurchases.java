...
import org.eclipse.store.demo.applogs.data.Purchase;
import org.eclipse.store.demo.applogs.data.PurchaseItem;
import org.eclipse.store.demo.applogs.data.Purchases;
import org.eclipse.store.demo.applogs.data.Shop;
...
public class ViewPurchases extends ViewEntity<Purchase>
{
	...
	protected void createUI()
	{
		...
		final Range<Integer> years = AppLogsDemo.getInstance().data().purchases().years();
	...
	{
		return AppLogsDemo.getInstance().data().purchases().computeByYear(
			this.year,
			function
		);
	}
	...
	public void listEntities() {
		...
		try {
			...
			this.totalColumnFooter.setText(
					AppLogsDemo.MONETARY_AMOUNT_FORMAT.format(stats.getSum()));
