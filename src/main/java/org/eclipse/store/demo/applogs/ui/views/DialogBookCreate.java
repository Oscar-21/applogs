...
import static org.eclipse.store.demo.applogs.ui.data.DataBindingUtils.setterDummy;
import static org.eclipse.store.demo.applogs.ui.data.DataBindingUtils.validator;
...
import org.eclipse.store.demo.applogs.data.Author;
import org.eclipse.store.demo.applogs.data.Book;
import org.eclipse.store.demo.applogs.data.Books;
import org.eclipse.store.demo.applogs.data.Genre;
import org.eclipse.store.demo.applogs.data.Language;
import org.eclipse.store.demo.applogs.data.Publisher;
import org.eclipse.store.demo.applogs.ui.data.DoubleToMonetaryAmountConverter;
...
public class DialogBookCreate extends Dialog
{
	...
	)
	{
		...
		final Button okButton = new Button(this.getTranslation("ok"), event ->
		{
			...
			{
				final MonetaryAmount purchasePrice = AppLogsDemo.money(purchasePriceField.getValue());
				final Book book = new Book(
					...
					purchasePrice,
					AppLogsDemo.retailPrice(purchasePrice)
				);
				this.close();
	...
	private static String generateIsbn13()
	{
		...
		final Books books = AppLogsDemo.getInstance().data().books();
		String      isbn;
	...
	private String validateIsbn13(final String isbn13)
	{
		...
		if(AppLogsDemo.getInstance().data().books().ofIsbn13(isbn13) != null)
		{
